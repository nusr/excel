...
export const ToolbarContainer: React.FunctionComponent<Props> = ({ controller }) => {
  ...
  return (
    <div className={styles['toolbar-wrapper']} data-testid="toolbar">
      <Button disabled={!controller.canUndo} onClick={() => controller.undo()} testId="toolbar-undo">
        <Icon name="undo" />
      </Button>
      <Button disabled={!controller.canRedo} onClick={() => controller.redo()} testId="toolbar-redo">
        <Icon name="redo" />
      </Button>
      ...
      />
      <Select data={FONT_SIZE_LIST} value={fontSize} onChange={(value) => setCellStyle({ fontSize: Number(value) })} />
      <Button active={isBold} onClick={() => setCellStyle({ isBold: !isBold })} testId="toolbar-bold" title="Bold">
        <Icon name="bold" />
      </Button>
      <Button
        active={isItalic}
      ...
      />
      <ColorPicker key="fill-color" color={fillColor} onChange={(value) => setCellStyle({ fillColor: value })}>
        <FillColorIcon />
      </ColorPicker>
      <ColorPicker key="font-color" color={fontColor} onChange={(value) => setCellStyle({ fontColor: value })}>
        <Icon name="fontColor" />
      </ColorPicker>
      <Button
        active={isWrapText}
