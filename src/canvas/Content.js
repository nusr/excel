import { isEmpty, thinLineWidth, npx, dpr, intToColumnName, Range, theme, canvasLog, } from '@/util';
import { fillRect, fillText, drawLines, renderCell, drawTriangle, resizeCanvas, } from './util';
...
export class Content {
    ...
    render({ changeSet }) {
        ...
        const check = changeSet.has('content') ||
    ...
    renderContent() {
        ...
        for (let r = row; r < endRow; r++) {
            ...
            for (let c = col; c < endCol; c++) {
                const cellInfo = controller.getCell(new Range(r, c, 1, 1, currentSheetId));
    ...
    renderTriangle() {
        ...
        const offset = 2;
        drawTriangle(this.ctx, [headerSize.width / 2 - offset, headerSize.height - offset], [headerSize.width - offset, headerSize.height - offset], [headerSize.width - offset, offset]);
        this.ctx.restore();
    }
