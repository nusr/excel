...
export class CollaborationProvider {
  ...
  async syncData() {
    ...
    await this.localDB.syncData(async (documentList, historyList) => {
      await this.remoteDB?.from('document').insert(documentList);
      await this.remoteDB?.from('history').insert(
        historyList.map((v) => {
          const { id: _id, ...rest } = v;
          return rest;
        }),
  ...
  async addDocument(): Promise<string> {
    ...
    const r = await this.remoteDB
      ?.from('document')
      .insert([{ name: '', sync: true }]);
    collaborationLog('db addDocument', r);
    // TODO: check r.data.id
    // @ts-ignore
